...
print(check_output(["ls", "C:/Users/KANG JUN HU/Documents/Google Drive/3.Purdue_Capstone_Design_Project/XLargeKSQ/sourcecode/input"]).decode("utf8"))

# Any results you write to the current directory are saved as output.
from sklearn.model_selection import KFold
data_path="C:/Users/KANG JUN HU/Documents/Google Drive/3.Purdue_Capstone_Design_Project/XLargeKSQ/sourcecode/input/"
df_test = pd.read_csv(data_path + 'text.csv',dtype={'msno' : 'category',
												'source_system_tab' : 'category',
                                                  'source_screen_name' : 'category',
                                                  'source_type' : 'category',
                                                'song_id' : 'category'})

df_train = pd.read_csv(data_path + 'train.csv',dtype={'msno' : 'category',
													'source_system_tab' : 'category',
                                                  'source_screen_name' : 'category',
                                                  'source_type' : 'category',
                                                  'target' : np.uint8,
...
df_members = pd.read_csv(data_path + 'members.csv',dtype={'city' : 'category',
                                                      'bd' : np.uint8,
                                                      'gender' : 'category',
...
df_members.drop('registration_init_time',axis=1,inplace=True) #registration_init_time을 지운다 axis는 컬럼을 의미 inplace트루이면 none으로 대체
...
df_songs = pd.read_csv(data_path + 'songs.csv',dtype={'genre_ids': 'category',
                                                  'language' : 'category',
