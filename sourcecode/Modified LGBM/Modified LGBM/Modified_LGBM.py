...
#extra feature5: featuring 여부 추가
def is_featured(x):
    if 'feat' in str(x):
        return 1
    return 0
...
train['is_featured'] = train['artist_name'].apply(is_featured).astype(np.int8) #is_featured에다가 추가한다.
test['artist_name'] = test['artist_name'].cat.add_categories(['no_artist'])
test['artist_name'].fillna('no_artist', inplace=True)
test['is_featured'] = test['artist_name'].apply(is_featured).astype(np.int8)

#extra feature6: 아티스트 수 체크
def artist_count(x):
    if x == 'no_artist':
        return 0
    else:
...
#extra feature7: 아티스트와 작곡가가 같은 지
...
#extra feature7: 아티스트와 작곡가, 작사가가 같은 지
...
# extra feature8: 17,45가 target이 1이 높아서 추가함
